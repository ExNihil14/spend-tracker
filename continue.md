...
- ⏳ **WIP (не закоммичено): калибровка порога 0.9 — инструмент готов** (`reports.confidence_calibration` +
...
> **АКТУАЛЬНО (15.09): 140 unit + 13 e2e зелёные, ruff чист; `main` = `65d5b61` (Фаза 3 /settings, запушена юзером). WIP: инструмент калибровки порога 0.9 (CLI `confidence`, ревью OpenRouter пройдено) — ждёт команды на коммит.** Ниже — исторические снимки; цифры в них не актуальны.
