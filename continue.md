...
> смоуки лендинга и приложения (0 ошибок консоли).
> **Волна M-2 «числа и язык» — СДЕЛАНА (ждёт коммита):** `fmt_money` (NBSP-разряды, запятая, ₽/код валюты,
> `signed=False` для лимитов/бюджетов), `fmt_month` («Сентябрь 2026»), `fmt_date` («13.09»); RU
> `display_name` для всех 18 категорий + `catname()` в UI (слаг остаётся ключом БД/URL/API); «возможный
> дубль» вместо near-дубля; детали аномалий — в fmt_money; placeholder суммы «−23,45» (парсер уже принимает
> запятую). Контур: **482 unit + 45 e2e** + ruff + contract (baseline осознанно обновлён: +4 аддитивных
> символа) + build_css + смоук всех страниц (форматы/категории/месяцы, 0 ошибок консоли); тесты обновлены
...
> **Ресёрч «какой слой проседает» (субагент-архитектор, 23.09) + фиксы слоя импорта (ждут коммита):**
> артефакт `D:\dev\docs\machine\RESEARCH_WEAK_LAYERS_SPENDTRACKER.md` (11 слоёв, оценки/доказательства,
> рейтинг проседания). **Топ-1 — импорт/экспорт** (moat проверен только синтетикой + техдефекты);
> топ-2 — веб-слой (форма 500, расхождения лендинг↔UI); топ-3 — доки/операционка (нет наблюдаемости).
> Закрыты S-фиксы из ресёрча: ① **атомарность партии** — `pseudonymize`/`merchant_cache_set`/LLM-кэш
> больше не коммитят внутри батча (сквозной `commit=False` через `categorize_transaction`/
> `classify_with_injectable`; регресс: откат строк+псевдонимов при сбое в середине); ② **CLI-импорт** —
> `read_bytes()` (работает cp1251-фолбэк) + реальное имя файла в партию (`import_csv(..., filename=…)`);
> ③ **форма без обязательного поля** → 422 (общий `_validation_422`), не 500. Контур: **509 unit +
> 48 e2e** + ruff + contract (baseline осознанно: +5 аддитивных сигнатур). Остаток ресёрча: 3 обезличенные
> golden-фикстуры (M), #15 (автор), bulkhead/наблюдаемость LLM, property-based.
> **Ресёрч публичных образцов выписок (23.09, по запросу «нет выписок — искать в инете») + golden-shape
> фикстуры (ждут коммита):** артефакт `D:\dev\docs\machine\RESEARCH_BANK_STATEMENT_SAMPLES.md` (первоисточники:
> ofxstatement-russian, moneymanagerex #1859, tinkoff.py, our-money, yoomoney docs). **Подтверждены реальные
> форматы:** Сбер email-CSV (12 колонок: `Тип карты;Номер карты;Дата совершения операции;…;Сумма в валюте
> счета`; `;`, utf-8, суммы с запятой, edge `-,01`) и Т-Банк (13 колонок с `MCC/Кэшбэк/Бонусы`, cp1251,
> `Статус=OK`). **Наши адаптеры это не понимали** → исправлено: синонимы Сбера (`Дата совершения операции`,
> `Сумма в валюте счета/операции`; расчётная сумма — в валюте счёта → RUB), Т-Банк (`Дата операции`, MCC-сниффинг
> раньше Сбера, скип не-OK статусов, `Номер карты`/`Валюта операции`), `missing_columns` — группы синонимов
> « / ». Golden-shape синтетика `gen_sber_email`/`gen_tinkoff_real` + 3 теста (сниффинг/импорт/cp1251/edge).
> **Факты для продукта:** у физлиц Сбера XLS/XLSX **не существует** (2021 → PDF; «Excel-лист» = CSV) →
> #4 data-gated окончательно; ЮMoney CSV кошелька публично не подтверждён (PDF/JSON). **Решение юзера (23.09):
> ✅ публичная формулировка исправлена** — README/лендинг: «у физлиц Сбер присылает CSV по e-mail и не у всех,
> выписка — PDF», обещание «XLS-импорт в планах» убрано, тест-гард `"xls-импорт" not in`. Контур: **512 unit +
> 48 e2e** + ruff + contract + build_css.
> **Волна M-4 «/approve: безопасный bulk + уверенность + клавиатура» — СДЕЛАНА (ждёт коммита):**
> ① кнопка «Одобрить все с уверенностью ≥ 60% (N)» — secondary, `hx-confirm` с числом и предупреждением;
> endpoint принимает `min_confidence` 0..1 (вне диапазона/не число → 422), без параметра — вся очередь
> (API-совместимость); `Store.approve_all_reviews(min_confidence=0.0)` — один UPDATE с порогом, кэш-INSERT OR
> IGNORE не тронут; ② уверенность — `conf_level()` («низкая/средняя/высокая», границы 0.5/0.7) + мини-бар
> с `aria-label`; ③ дубль «чип + select» убран — предложение подсвечено в самом select («— предложено»),
> сноска «сейчас: X» осталась; «Пропустить» — текстовая ссылка; ④ `static/approve.js`: `j/k` (фокус на
> кнопке строки + `.is-selected`), `Enter` (нативно), `s` — skip, `1–9` — категория; после свопа фокус
> возвращается на ту же позицию; ввод в select/input не перехватывается; ⑤ мерчант не дублируется, если
> входит в описание. QA-план обновлён (TC-03/04/08 + TC-16/17/18), PIPELINE/CONTEXT. Контур: **522 unit +
> 49 e2e** + ruff + contract (baseline осознанно: +1 сигнатура, +1 параметр) + build_css.
> Следующее по плану: M-5 (/dashboard: аномалии наверх, сортировка по риску, маркер темпа, дельты словами),
> затем M-6 (/transactions: таблица выше форм + file input), M-7 (nav/settings) — по команде юзера.
> **Волна M-5 «/dashboard: аномалии наверх, риск, темп, дельты словами» — СДЕЛАНА (ждёт коммита):**
> ① аномалии — отдельная карточка `#anomalies-card` ПЕРВЫМ блоком (иконка типа SVG, мерчант + label/дата +
> деталь, сумма справа, «Открыть» → `/?q=<мерчант>`); из карточки дайджеста подсекция убрана; ② дельты
> расходов словами — `delta_words()` («↑ на 6 925,00 ₽» / «↓ на …» / «без изменений», рост трат красный),
> «Δ» в UI больше нет; ③ бюджеты сортируются по риску (`budgets_progress`: pct desc, tie-break имя) +
> маркер темпа — вертикальная риска `_month_elapsed_pct()` (доля прошедшего месяца; прошлый — 100, будущий — 0)
> и легенда «риска — прошло N% месяца»; ④ «это ок» для аномалий сознательно НЕ делаем — дайджест read-only
> без состояния (отдельная задача, если понадобится). Тесты: +1 unit delta_words, +1 порядок бюджетов,
> +2 дашборд-UI (карточка/порядок/маркер/Δ), e2e дайджеста обновлён (аномалии — своя карточка). Контур:
> **525 unit + 49 e2e** + ruff + contract (baseline: +delta_words) + build_css. Заодно: закоммичены M-4
> (`ac6c7f1`) и XLS-формулировка (`ad4bd32`), прод рестартнут и проверен живьём (Клавиши/bulk-кнопка/approve.js).
> Следующее: M-6 (/transactions: таблица выше форм + file input), M-7 (nav/settings).
> **Волна M-6 «/transactions: данные выше форм + импорт файлом» — СДЕЛАНА (ждёт коммита):**
> ① итоги месяца — строкой (Доход/Расход/Баланс + стрелки), KPI-карточки остались только на дашборде
> (дубль сжат на одной странице); ② «Импорт CSV» и «Добавить» свёрнуты в кнопки шапки таблицы
> (`[data-toggle]` → `#import-panel`/`#add-panel`, `hidden` + `aria-expanded/controls`, JS в app.js;
> `/#import`, `/#add` открывают панель через hash — обещания лендинга/чек-листа/дашборда сохранены);
> ③ импорт файлом: `<input type=file>` + drop-zone (drag&drop подставляет файл), textarea — запасной путь;
> `hx-encoding=multipart`; API `/api/import` принимает multipart (байты → utf-8-sig/cp1251-фолбэк,
> имя файла в партию, пустой file → textarea). Тесты: +3 multipart (utf-8/cp1251/пустой), +1 структура
> главной, e2e `open_panel()` + загрузка файла. Контур: **529 unit + 50 e2e** + ruff + contract + build_css.
> Следующее: M-7 (nav активное состояние + /settings автосейв цвета), затем порядок секций лендинга (M-8).
> **Волна M-7 «nav + /settings + типографика» — СДЕЛАНА (ждёт коммита):** ① nav — пункты приглушены,
> активный `text-fg-strong` + подчёркивание 2px accent (по `request.url.path`); ② /settings — цвет категории
> и лимит бюджета сохраняются по `change` (кнопки «OK» убраны), ответ — фрагмент + OOB-тост «Сохранено»
> (общий `spendtrack/ui.py: oob_toast`, api.py переведён на него же — тост заодно с палитры slate на токены);
> у «Удалить» disabled title «Используется (N) — сначала перенесите…»; ③ типографика P1-5: UPPERCASE только
> у заголовков колонок, заголовки карточек — sentence case 15px/600, мелкие подписи без uppercase.
> Тесты: +2 settings (автосейв/тост/title), +2 UI (nav active/sentence case), +1 e2e (цвет → тост), бюджет-e2e
> переведён на blur-автосейв. Контур: **533 unit + 51 e2e** + ruff + contract (baseline: +oob_toast) + build_css.
> Следующее: M-8 (порядок секций лендинга + финальный CTA) — последняя волна плана.
> **Ревью волн M-4…M-7 (возобновлено 23.09, $0):** `D:\dev\docs\machine\EXPERT_REVIEW_M_WAVES_OSS_2026-09-23.md`
> — gpt-oss:120b (ollama-cloud free, 13 с, 8.5K/3.7K ток.): **GO**, реальных дефектов 0; 2 «P1» (cat_colors)
> отклонены фактами (шаблоны переписаны в M-4, e2e/unit зелёные), P2 — косметика/ложные. Урок каналов:
> thinking-модели на free жгут весь max_tokens в reasoning (nemotron-3-ultra → пустой ответ); для ревью —
> `gpt-oss:120b` (ответ отдельно), kilo транзиентно отдаёт ответ без `choices`, OVH — 2 RPM/IP.
> Практика: $0-ревью на волну (дифф по коду) + тяжёлое ревью на вехах.
> **Волна M-8 «лендинг: порядок секций + финальный CTA» — СДЕЛАНА (ждёт коммита; последняя волна плана):**
> ① порядок под путь доверия: hero → facts → «Что вы увидите» (3 кадра: импорт → очередь → дайджест,
> `#screens`) → **privacy ДО установки** → install → features → «LLM предлагает, вы решаете» (без дубля
> скриншота, +пункт про клавиатуру) → demo → support → FAQ → roadmap → **финальный CTA** (команда установки
> + демо) → EN; ② формулировки «с результата»: «Выписки без ручной правки», «Категории расставляются сами»,
> «Загрузили дважды — дублей нет», «ИИ выключен, пока вы сами не включите», near-дубли → «возможные дубли»;
> ③ скриншоты пересняты после M-4…M-7 (4 шт., 2x, стенд 8767) — очередь/дайджест/импорт/hero-кроп;
> ④ фикс горизонтального скролла лендинга на 1280px (сетки `minmax(0, 1fr)` + `min-width:0`).
> Проверки: test_landing +2 (порядок/CTA, формулировки; всего 11), браузерный смоук (4 img naturalWidth>0,
...
> нашими процессами (№6 «тупая модель + жёсткий цикл проверки» = контракт-дельта/тесты/ruff после AI-правок;
> №4 Cost per Task = go-usage/go-report + роутинг моделей; №12 архитектурная целостность = REVIEW_CHECKLIST/
> data-discipline; №13 CI FastAPI = наш CI уже покрывает lint/e2e/contract/audit, VPS-деплой не применим
> (local-first)). **Реально новые кандидаты:** №2 — фиксировать «definition of value» (метрика успеха) в
> плане задачи; №5 — `llama.cpp` вместо Ollama для бандлинга офлайн-LLM (бэклог); №15 — KV-cache стриминг
> (прочитать); №1/№9/№16 — бэклог-эксперименты (локальный 80B MoE, chaos-тест импорта, guard-модель —
> у нас уже схема-валидация). Следующее по Habr: при желании внедрить «definition of value» в шаблон
> брифа/плана (S, docs).
