...
- **Порядок и защита ротации (§J-2 critical, 26.09):** `--keep < 1` — отказ (`rc=1`) до создания снимка;
  offsite-копия делается **до** ротации (отказ копии на том же томе не удаляет старые снимки); свежий снимок
  не удаляется никогда и занимает один из слотов `keep` (при `--keep 1` локально остаётся ровно он);
  порядок удаления — по `(mtime, name)` — единый критерий с `doctor.check_backup` (сортировка по имени
  ошибалась на UTC-именах и дублях секунды `…-1.db`); `OSError` при удалении (Windows: файл занят
  антивирусом/индексатором) — предупреждение в stderr, `rc=0` (бэкап уже создан и скопирован).
- Инструкция — README/FAQ + /help; `doctor` вызванный без настройки offsite даёт info, не ошибку.
- Тесты: `tests/test_backup_offsite.py` (16, оффлайн; guard/force/sha-мисматч/недоступная папка/
  путь-файл/ротация не трогает внешнюю папку, `--keep < 1`, копия-до-ротации, OSError ротации, mtime-порядок,
  защита свежего снимка) + offsite-тесты `tests/test_doctor.py` (6).
...
- **Честность гейта (§J-2 critical, 26.09):** отсутствие/нечисло гейтируемой метрики в базлайне или замере,
  битый/отсутствующий базлайн, неизвестная версия схемы и упавший `measure()` — **FAIL** в обоих режимах
  (раньше молчаливый `exit 0`; в текстовом режиме — `TypeError` вместо FAIL). Значение 0 или < 0.5×базлайна —
  тоже FAIL («слишком хорошо»: так выглядит поломка замера, а не оптимизация). `measure()` несёт ассерты-
  инварианты: seed 480 строк, отчёт за 2026-06 непустой, импорт synth-выписки `status=ok`/104 строки
  (100 базовых + 4 фикстуры генератора: зарплата/возврат/дубль дня/«В обработке»).
- Базлайн 25.09: отчёт — **2** запроса; импорт 100 строк — **435** запросов (~4.35/строку — кандидат в K7).
- Тесты: `tests/test_ratchet.py` (11, оффлайн: форма baseline, детект регресса, отсутствующие/нечисловые
