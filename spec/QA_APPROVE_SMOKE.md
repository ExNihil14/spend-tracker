...
### TC-03 (P1) Diff «предложение LLM vs категория в БД» (M-4: чип убран)
- Строка «СТРОЙКАОПТ МСК…»: select показывает «Быт и дом — предложено» (подсветка предложения в самом select), ниже серая сноска «сейчас: Прочее».
- Строка «ПЕРЕВОД ДРУГУ ИВАН»: `category_llm == category == transfers` → сноски НЕТ.
- Критерий: сноска появляется только когда `category_llm != category`; отдельного бейджа-дубля нет.

### TC-04 (P1) Отображение суммы (M-3: расходы нейтральные)
- Отрицательные суммы — нейтральный текст со знаком «−» («−1 234,50 ₽»), положительная «ВОЗВРАТ OZON» — зелёным (`text-income`).
- Критерий: знак несёт смысл без цвета (WCAG 1.4.1); формат `fmt_money` (разряды, запятая, ₽).
...
### TC-08 (P1) Пакетное одобрение с порогом (M-4)
- Шаги: нажать «Одобрить все с уверенностью ≥ 60% (N)» → подтвердить диалог (в тексте — число записей и предупреждение, что слабые останутся).
- Ожидаемо: исчезают только строки с `confidence ≥ 0.6`; слабые остаются; если уверенных больше нет — кнопка исчезает.
- Критерий: одобренные получили `category=category_llm` (или `category`, если предложения не было); у оставшихся `review_status='pending'`.
- Примечание: без параметра `min_confidence` endpoint по-прежнему одобряет всю очередь (API-совместимость, тесты).

### TC-16 (P1) Уверенность текстом и мини-бар (M-4)
- Строка с `confidence=0.65`: подпись «средняя» + мини-бар заполнен на 65%, `aria-label="Уверенность 65%"`.
- Границы: `<0.5` — «низкая», `0.5–0.69` — «средняя», `≥0.7` — «высокая».
- Критерий: процент читается ассистивно, визуально — уровень + бар (не «0.00%»).

### TC-17 (P1) Клавиатурный триаж (M-4)
- Шаги: `j` — выбрать строку (подсветка `is-selected`, фокус на «Одобрить»), `Enter` — одобрить, `s` — пропустить, `1–9` — категория по номеру.
- Ожидаемо: после approve/skip фокус остаётся на той же позиции (следующая строка); ввод в select/input не перехватывается.
- Критерий: очередь можно разобрать без мыши; e2e `test_approve_keyboard_triage`.

### TC-18 (P2) Строка мерчанта не дублируется (M-4)
- Строка «ФОТОЛАБ ОПЛАТА» (merchant `ФОТОЛАБ` уже в описании) — второй строки с мерчантом НЕТ.
- Строка «ОПЛАТА КАРТОЙ» (merchant `ФОТОЛАБ` не в описании) — вторая строка ЕСТЬ.
- Критерий: мерчант показывается только когда добавляет информацию.
...
6. Клавиатурный триаж (TC-17) разбирает очередь без мыши; пакетное одобрение не трогает записи ниже порога (TC-08).
