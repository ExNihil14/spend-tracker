...
## Термины
- **Транзакция** — одна операция (доход/расход) из банка или CLI. Поля: date, description, amount_kopecks (INTEGER копеек), category, category_source, confidence, merchant, account_anon, import_batch, fingerprint.
...
- **Подтверждение (review queue)** — транзакции с `category_source='llm_pending_review'` ИЛИ `(confidence<1.0 AND category_source NOT IN ('rule','import','manual'))`. Валидируются вручную.
